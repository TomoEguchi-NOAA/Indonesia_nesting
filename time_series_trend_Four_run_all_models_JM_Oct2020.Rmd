...
```{r}
jags.data <- list(y = data.jags.JM$jags.data2$y,
                  ...
                  pi = pi,
                  period = period.JM,
                  N0_mean = log(sum(exp(data.jags.JM$jags.data2$y[1,]), na.rm = T)),
                  N0_sd = 10,
                  n.years = nrow(data.jags.JM$jags.data2$y))
...
for (k2 in 1:length(obs.models)){
  for (k3 in 1:length(state.models)){
    for (k4 in 1:length(v2.models)){
      ...
      filename.root <- paste0("SSAR1_", state.model, "_", obs.model, "_trend_", v2.model, "_JM", 
                              period.JM, "_",
      ...
      if (file.exists(model.name)){
        ...
        if (!file.exists(paste0("RData/", filename.root, '.rds'))){
          ...
          ys.stats$time <- data.jags.JM$data.1$Frac.Year
          ys.stats$obsY <- data.jags.JM$data.1$Nests
          ys.stats$month <- data.jags.JM$data.1$Month
          ys.stats$year <- data.jags.JM$data.1$Year
          ys.stats$Season <- data.jags.JM$data.1$Season
          ys.stats$location <- "Jamursba-Medi"
          ...
          if (v2.model == "Four"){
            ...
            Xs.stats$time <- data.jags.JM$data.1$Frac.Year
            Xs.stats$obsY <- data.jags.JM$data.1$Nests
            Xs.stats$month <- data.jags.JM$data.1$Month
            Xs.stats$year <- data.jags.JM$data.1$Year
            Xs.stats$Season <- data.jags.JM$data.1$Season
            Xs.stats$location <- "Jamursba-Medi"
          }          
          ...
          Ns.stats$location <- "Jamursba-Medi"
...
The models without the Xs showed better Pareto k values (all k < 0.5). Compare LOOIC values for those models. 
