...
signif(results.JM_SSAR1_month_var_theta$g.diag$mpsrf, digits = 4)```, indicating an adequate convergence, though not great. Visual inspection of MCMC chains also indicated that a convergence was reached for all parameters (Figure 7). 
...
Because the annual counts are computed for nesting seasons, monthly counts are summed from April 1 to March 31 over a two-year peiord.
...
data.1.JM.1999 %>% mutate(Season = ifelse(MONTH < 4, YEAR - 1, YEAR),
                    season = ifelse(MONTH > 3 & MONTH < 10, 
                                    "summer", "winter")) %>%
  group_by(Season) %>%
...
# yrs <- as.matrix(c(min(Xs.stats$Season):max(Xs.stats$Season)))
...
data.1.W.2006 %>% mutate(Season = ifelse(MONTH < 4, YEAR - 1, YEAR),
                         season = ifelse(MONTH > 3 & MONTH < 10, 
                                         "summer", "winter")) %>%
  group_by(Season) %>%
  summarise(observed = sum(count, na.rm = T)) %>%
