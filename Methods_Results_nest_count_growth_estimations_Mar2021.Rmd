...
"There was also a 62.8% decline at Wermon from 2,994 nests in 2002 to 1,096 in 2011 (GLM, r^2 = 0.78, p < 0.01;Table 1, Fig. 3). In comparison, during our current study (2005–2011), nesting declined 29.0% at Jamursba Medi (GLM; r^2 = 0.92, p < 0.01), and 52.2% at Wermon (GLM; r^2 = 0.52, p < 0.01)."

```{r compare_Tapilatu_W, echo=FALSE}
Tapilatu.W.data <- data.frame(Year = c(2002:2011),
...
data.jags.W$data.0 %>% 
  ...
  select(Season, Nests) %>%
  group_by(Season) %>%
  summarize(Nests = sum(Nests, na.rm = T)) -> W.data.year
...
Tapilatu.W.data.2 <- rbind(Tapilatu.W.data, W.data.year.2012) %>%
  mutate(Year.shifted = Year - min(Tapilatu.W.data$Year))
...
# Move the year so the first year is zero.
Tapilatu.W.data %>% mutate(Year.shifted = Year - min(Year)) -> Tapilatu.W.data

Tapilatu.W.fit <- lm(log(Nests) ~ Year.shifted, data = Tapilatu.W.data)
...
```{r Tapilatu_fit_2005to2011, echo=FALSE}
Tapilatu.W.data %>% filter(Year > 2004) %>% 
  mutate(Year.shifted = Year - min(Year)) -> Tapilatu.W.data.2005

Tapilatu.W.fit.2005 <- lm(log(Nests) ~ Year.shifted, data = Tapilatu.W.data.2005)
...
```{r Tapilatu_fit_W_2012, echo=FALSE}
Tapilatu.W.fit.2 <- lm(log(Nests) ~ Year.shifted, data = Tapilatu.W.data.2)
#summary(Tapilatu.fit.2)
Tapilatu.W.data.2$fitted <- Tapilatu.W.fit.2$fitted.values

```

When eight more years of data were added (2012-2019) to the dataset in Tapilatu et al. (2013), estimated growth rate increased by ```r signif(abs(Tapilatu.W.fit.2$coefficients[2] - Tapilatu.W.fit$coefficients[2]) * 100, 3)``` % per year (U = ```r signif(Tapilatu.W.fit.2$coefficients[2], 3)```, SE = ```r signif(summary(Tapilatu.W.fit.2)$coefficients[2,2], 3)```). This is equivalent of ```r signif(((1 - exp(Tapilatu.W.fit.2$coefficients[2] * 17)) * 100), 3)``` % decline over the 17 years.

```{r Tapilatu_new_regression_W, echo=FALSE}
p.GLM.W <- ggplot() + 
  ...
  geom_line(data = Tapilatu.W.data,
            aes(x = Year, y = exp(fitted)),
            color = "orange") +
  geom_point(data = Tapilatu.W.data.2 %>% filter(Year > 2011),
             aes(Year, y = Nests),
             color = "blue") +
...
```{r plot_GLM_W, echo=FALSE, cache=TRUE, fig.cap = "Fitted general linear models to the Wermon data in Tapilatu et al. 2013 (orange), with new data (blue)."}
knitr::include_graphics("figures/GLM_fit_W.png")
```

The numbers of nests since 2014 may indicate potential increase (Figure x).
...
```{r BayesianRegression_W, echo=F}
# Use W data only and run the models
loo.out.W <- readRDS(file = "RData/loo_all_W_only_Mar2021.rds")
filenames.W <- readRDS(file = "RData/filenames_W_only_Mar2021.rds")

pareto.k.W <- lapply(loo.out.W, 
                   FUN = function(x) x$loo.out)

# find maximum pareto k values
max.pareto.k.W <- unlist(lapply(pareto.k.W,
       FUN = function(x) max(x$diagnostics$pareto_k)))

# find the models that have max(pareto k) < 0.7
good.models.W <- filenames.W[which(max.pareto.k.W < 0.7)]
good.models.pareto.k.W <- pareto.k.W[which(max.pareto.k.W < 0.7)]
good.models.max.pareto.k.W <- max.pareto.k.W[which(max.pareto.k.W < 0.7)]

looic.esimates.W <- lapply(lapply(loo.out.W[which(max.pareto.k.W < 0.7)], 
                                FUN = function(x) x$loo.out),
                         FUN = function(x) x$estimates)

looic.W <- unlist(lapply(looic.esimates.W, 
                       FUN = function(x) x["looic", "Estimate"]))

loo.out.list.W <- lapply(loo.out.W[which(max.pareto.k.W < 0.7)], 
                       FUN = function(x) x$loo.out)

# calculate model weights
model.weights.W <- loo_model_weights(loo.out.list.W)

model.names.abb.W <- lapply(good.models.W, 
                          FUN = function(x) strsplit(x, split = "SSAR1_")[[1]][2]) %>%  
  lapply(FUN = function(x) strsplit(x, split = "_W6")[[1]][1]) %>% 
  lapply(FUN = function(x) strsplit(x, split = "trend_DFS_")) %>%
  lapply(FUN = function(x) paste(unlist(x), collapse = "")) %>% unlist() 

options(scipen = 999)

looic.table.W <- data.frame(model = model.names.abb.W,
                                 Max.Pareto = good.models.max.pareto.k.W,
                                 LOOIC = looic.W) %>% 
  mutate(delta.LOOIC = looic.W - min(looic.W)) %>%
  arrange(by = delta.LOOIC) %>%
  select(-LOOIC) %>%
  mutate_if(is.numeric, round, digits = 4)


best.model.W <- good.models.W[which(looic.W == min(looic.W))]
pareto.k.best.W <- good.models.pareto.k.W[[which(looic.W == min(looic.W))]]

second.best.model.W <- good.models.W[order(looic.W)[2]]

jm <- readRDS(file = paste0("RData/", best.model.W[[1]], ".rds"))
#max(unlist(lapply(jm$Rhat, FUN = max, na.rm = T)))
data.frame(jm$summary) %>% rownames_to_column("Parameter") -> summary.df

# check convergence 
rhat.max.W.1 <- max(unlist(lapply(jm$Rhat, FUN = max, na.rm = T)))

# look at some posteriors
U.stats.W.1 <- summary.df[grep(summary.df$Parameter, pattern = "U"),]
p.post.U.W.1 <- bayesplot::mcmc_dens(jm$samples, c("U")) + 
  xlab("Annual growth rate") + ylab("Density")

if (save.fig)
  ggsave(filename = 'figures/W_posterior_t_t_1U.png', 
         plot = p.post.U.W.1,
         dpi = 600, height = fig.height, 
         width = fig.width, 
         units = "in")
...
# Second best model:
jm <- readRDS(file = paste0("RData/", second.best.model.W[[1]], ".rds"))
#max(unlist(lapply(jm$Rhat, FUN = max, na.rm = T)))
data.frame(jm$summary) %>% rownames_to_column("Parameter") -> summary.df

# check convergence 
rhat.max.W.2 <- max(unlist(lapply(jm$Rhat, FUN = max, na.rm = T)))

U.stats.W.2 <- summary.df[grep(summary.df$Parameter, pattern = "U"),]
p.post.U.W.2 <- bayesplot::mcmc_dens(jm$samples, c("U")) + 
  xlab("Annual growth rate") + ylab("Density")

if (save.fig)
  ggsave(filename = 'figures/W_posterior_norm_t_1U.png', 
         plot = p.post.U.W.2,
         dpi = 600, height = fig.height, 
         width = fig.width, 
         units = "in")
...
Among the 8 models fit to the Wermon dataset, only ```r length(good.models.W)``` models were found appropriate based on the Pareto k statistic (Table x). The best model included Student's t distribution for state and observation processes, whereas the second best model was with the normal distribution for the state and Student's t distribution for the observation. Convergence was reached for both models, where maximum Rhat < ```r signif(ifelse(rhat.max.W.1 < 1.001, 1.01, rhat.max.W.1), 3)``` for the best one and Rhat < ```r signif(ifelse(rhat.max.W.2 < 1.001, 1.01, rhat.max.W.2), 3)``` for the second best model. 


```{r looic_table_W, echo=F}
knitr::kable(looic.table.W, 
             digits = 2,
             caption = "A comparison of models that were fit to the Wermon dataset and passed the goodness-of-fit test using Pareto k statistics.")
```

The estimated mean growth rate (U) at Wermon ```r signif(U.stats.W.1[1,"mean"], digits = 2)``` (SE = ```r signif(U.stats.W.1[1, "sd"], digits = 2)```, p(U < 0.0) = ```r signif(U.stats.W.1[1, "f"], digits = 2)```, Figure 12). The second best model showed similar results: ```r signif(U.stats.W.2[1,"mean"], digits = 2)``` (SE = ```r signif(U.stats.W.2[1, "sd"], digits = 2)```, p(U < 0.0) = ```r signif(U.stats.W.2[1, "f"], digits = 2)```.


```{r plot_posterior_W_1U, echo=FALSE, cache=TRUE, fig.cap = "The posterior distribution of the growth rate (U) at Wermon from the best model based on LOOIC. "}
knitr::include_graphics("figures/W_posterior_1U.png")
```
...
This turned out to be not quite feasible... here is the reason I explained to Manjula on 2021-03-16 via email:

"So, as I understood the analysis idea, you wanted to combine old data that I didn't use to impute missing data and new data with imputation to estimate the rate of population decline. This sounded feasible but it's not as easy as it sounded the first time. This is because it was not easy to come up with annual nest counts from the old data that I didn't use to impute missing values. The reason I didn't use them was because there were not enough data within each year. So, it gets a bit circular... Does this make sense? 

"Of course, we can use them as raw numbers without filling in missing data in those years but then they will not be comparable to later years when we have full year's data." 
