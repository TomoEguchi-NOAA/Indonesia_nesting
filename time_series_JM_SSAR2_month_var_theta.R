...
load.module('dic')
params <- c('theta1.1', 'theta1.2', 'theta2.1', 'theta2.2',
            'sigma.pro1', 'sigma.pro2', 'sigma.obs', 'mu')

jm <- jags.model(file = 'models/model_SSAR2_month_var_theta.txt',
                 data = bugs.data,
...
if (save.fig)
  ggsave(plot = p.1,
         filename = 'figures/predicted_counts_SSAR2_JM_month_var_theta_2000.png',
         dpi = 600)

if (save.RData)
  save(results.JM_SSAR1_month,
       file = paste0('RData/SSAR2_month_JM_var_theta_',
...
#
mcmc_dens(zm, c('theta1.1', 'theta1.2', 'theta2.1', 'theta2.2'))
