...
Trying to merge the data imputation process and trend analysis with multiple datasets. This is for norm_norm and norm_t with a discrete Fourier series for the means and Q (process variance) with independent Us and Qs.
...
Bring in the data (real or simulated)
...
if (!simulation){
  ...
  out.list <- readRDS("RData/sim_constCV_independentUQ_data_parameters_2019-07-03.rds")
  period.1 <- 12
  period.2 <- 6
...
filename.root <- paste0("SSAR1_norm_norm_trend_Four_FourQ_independentUQs_",
...
jm <- jags(jags.data,
           ...
           model.file = 'models/model_norm_norm_trend_Four_FourQ_independentUQs.txt',
...
```{r}
if (!simulation){
  data.1 <- data.jags.JM$data.1
  data.2 <- data.jags.W$data.1 
}
...
ys.stats.JM$time <- data.1$Frac.Year
ys.stats.JM$obsY <- data.1$Nests
ys.stats.JM$month <- data.1$Month
ys.stats.JM$year <- data.1$Year
ys.stats.JM$Season <- data.1$Season
...
ys.stats.W$time <- data.2$Frac.Year
ys.stats.W$obsY <- data.2$Nests
ys.stats.W$month <- data.2$Month
ys.stats.W$year <- data.2$Year
ys.stats.W$Season <- data.2$Season
...
Xs.stats.JM$time <- data.1$Frac.Year
Xs.stats.JM$obsY <- data.1$Nests
Xs.stats.JM$month <- data.1$Month
Xs.stats.JM$year <- data.1$Year
Xs.stats.JM$Season <- data.1$Season
...
Xs.stats.W$time <- data.2$Frac.Year
Xs.stats.W$obsY <- data.2$Nests
Xs.stats.W$month <- data.2$Month
Xs.stats.W$year <- data.2$Year
Xs.stats.W$Season <- data.2$Season
...
bayesplot::mcmc_dens(jm$samples, c("q.beta.cos[1]", "q.beta.sin[1]",
                                   "q.beta.cos[2]", "q.beta.sin[2]"))
