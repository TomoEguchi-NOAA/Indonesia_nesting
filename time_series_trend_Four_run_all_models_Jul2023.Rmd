...
# collect all U samples in one dataframe for plotting later:
df.1U <- data.frame(dataset = "Jeen Yessa and Jeen Syuab",
                    U = jm$sims.list$U,
                    Dataset.idx = "A")
...
df.summer.U <- data.frame(dataset = rep(c("Jeen Yessa Boreal Summer", "Jeen Syuab Boreal Summer"), 
                                        each = nrow(jm$sims.list$U)),
                          U = c(jm$sims.list$U[,1], jm$sims.list$U[,2]),
                          Dataset.idx = rep(c("B", "C"), each = nrow(jm$sims.list$U)))
...
df.Wermon.Winter <- data.frame(dataset = "Jeen Syuab Austral Summer",
                               U = jm$sims.list$U,
                               Dataset.idx = "D")
...
Create a plot for all Us
```{r}
library(ggh4x)
df.U <- rbind(df.1U, df.summer.U, df.Wermon.Winter)
