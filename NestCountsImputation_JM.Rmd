...
M <- readRDS(paste0("RData/jagsout_", str.root, "_JM_1999_2017_",
                    run.date, ".rds"))
...
#Model Comparison
First, we compared all models using using ELPD values although Pareto k statistics indicated that models 1, 2, 8, 11, 13, and 17 had acceptable predictive performance. 
...
Interesting to see that those models that are acceptable according to Pareto k statistics are worse ones according to looic scores. I will use M5 (the best according to looic alone), M11 (best looking at Pareto k statistics alone), and M1 (best among acceptable Pareto k statisitcs models). 

Model 5, 

Posterior distributions of thetas:

```{r posterior_M6_theta, fig.cap="Figure. Posterior distributions of slope parameters for M5"}
bayesplot::mcmc_dens(M5.jm$samples, c("theta.1[1]", "theta.1[2]", "theta.1[3]", "theta.1[4]",
...
```{r posterior_M6_theta, fig.cap="Figure. Posterior distributions of slope parameters for M5"}
bayesplot::mcmc_dens(M5.jm$samples, c("sigma.pro1[1]", "sigma.pro1[2]", 
...
```{r posterior_M11, fig.cap="Figure. Posterior distribuitons of slope parameters for M11"}
bayesplot::mcmc_dens(M11.jm$samples, c("theta.1", "theta.2"))
```

```{r posterior_M11_df, fig.cap="Figure. Posterior distributions of degrees-of-freedom parameter for M11"}

bayesplot::mcmc_dens(M11.jm$samples, "df")
```
The slope parameters indicated increasing trend to July and decreased from August through November. 

```{r posterior_M11_2, fig.cap="Figure. Posterior distributions for the standard deviation of the process (sigma.pro1) and standard deviation of observations (sigma.obs)."}
bayesplot::mcmc_dens(M11.jm$samples, c("sigma.pro1", "sigma.obs"))
```

In Model 1, 

```{r posterior_M1_theta, fig.cap="Figure. Posterior distributions of slope parameters for M1"}
bayesplot::mcmc_dens(M1.jm$samples, "theta.1")
```
...
```{r}
str.root <- strsplit(strsplit(as.character(models.df[1, 'names']), "model_")[[1]][2], ".txt")[[1]]
M1 <- readRDS(paste0("RData/jagsout_", str.root, "_JM_1999_2017_",
                    run.date, ".rds"))
...
```{r}
str.root <- strsplit(strsplit(as.character(models.df[5, 'names']), "model_")[[1]][2], ".txt")[[1]]
M5 <- readRDS(paste0("RData/jagsout_", str.root, "_JM_1999_2017_",
                    run.date, ".rds"))
...
```{r}
str.root <- strsplit(strsplit(as.character(models.df[11, 'names']), "model_")[[1]][2], ".txt")[[1]]
M11 <- readRDS(paste0("RData/jagsout_", str.root, "_JM_1999_2017_",
                    run.date, ".rds"))

Xs.stats <- M11$Xs.stats
ys.stats <- M11$ys.stats
p.1 <- ggplot() +
  #geom_point(data = ys.stats,
  #           aes(x = time, y = mode_y), color = "blue") +
  #geom_line(data = Xs.stats,
...
Qualitatively... M11 seems to provide "better" predictions. So, go with that. 
