...
```{r}
jags.params <- c('N', 'U', "p", "p_beta_cos", "p_beta_sin",
                 "sigma_N", 'sigma_Q', "sigma_R",
                 "mu", "y", "X", "deviance", "log_lik")
...
```{r}
# rstan and stan are having issues as of 2019-07-10. Will come back to this later. 

mod_monthQ_independentUQs <- stan("models/model_norm_norm_trend_Four_monthQ_independentUQs.stan", 
                                  data = data.list, 
...
log_lik_monthQ_independentUQs <- extract_log_lik(mod_monthQ_independentUQs, 
                                   merge_chains = FALSE)

rel_n_eff_monthQ_independentUQs <- relative_eff(exp(log_lik_monthQ_independentUQs))
loo__monthQ_independentUQs <- loo(log_lik_monthQ_independentUQs, 
                                  r_eff = rel_n_eff_monthQ_independentUQs, 
                                  cores = n.cores)
...
if (save.data)
  saveRDS(results.all,
          file = paste0("RData/", "norm_norm_trend_Four_monthQ_independentUQs_stan_", 
                        Sys.date(), '.rds'))
```
