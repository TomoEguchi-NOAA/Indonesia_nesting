...
# M <- readRDS(paste0("RData/jagsout_SSAR1_logY_norm_t_varM_thetaM_W_2006_2014_", 
#                     run.date, ".rds"))
M19.loo.out <- M$jags.out$loo.out$loo.out
M19.jm <- M$jags.out$jm
...
#Model comparison
Estimated Pareto k statistics indicated that models 2, 7, 8, 11, 16, and 17 had better predictive performance than the rest. But when looking at the looic values, those are ranked low:
...
Among the "good" k pareto statistics models, M7 and M16 are the best from looic values. So, let's take a look at the posteriors. 

Model 7, 

The posterior distributions of the model are shown below. First, the slope parameter: one for each month.  Negative for Feb - Apr and Jul - Oct and positive for May - June and Nov-Jan. 

```{r posterior_M7, fig.cap="Figure. Posterior distribuitons of slope parameters for M7"}
m <- 7
best.model <- M7.jm
bayesplot::mcmc_dens(best.model$samples, c("theta.1[1]", "theta.1[2]", "theta.1[3]", "theta.1[4]",
                                      "theta.1[5]", "theta.1[6]", "theta.1[7]", "theta.1[8]",
                                      "theta.1[9]", "theta.1[10]", "theta.1[11]", "theta.1[12]"))
```

Standard deviation of the process (sigma.pro1 and sigma.pro2) and observation (sigma.ops).  Two process SDs are for the "nesting" season and "non-nesting" season.  

```{r posterior_M7_2, fig.cap="Figure. Posterior distributions for the standard deviation of the process (sigma.pro1), standard deviation of observations (sigma.obs), and degrees of freedom (df) parameter."}
bayesplot::mcmc_dens(best.model$samples, c("sigma.pro1", "sigma.pro2", "sigma.obs"))
```
...
if (!file.exists(paste0('RData/', "jagsout_", 
  ...
  jags.out <- run.jagsUI(data.jags$jags.data, 
                         jags.params, 
                         model.file = paste0("models/", as.character(models.df[m, "names"])))
  ...
  results <- list(data.1 = data.jags$data.1,
                  ...
                  Xs.stats = Xs.stats,
                  ys.stats = ys.stats,
                  MCMC.params = jags.out$MCMC.params,
                  time = toc - tic)
...
ggsave(p.1,
       filename = "figures/W_imputed_2006_2018_M7.png",
       device = "png", dpi = 600,
       height = 4, width = 6, units = "in")
