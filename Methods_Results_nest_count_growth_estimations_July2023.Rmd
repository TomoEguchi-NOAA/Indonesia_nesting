...
run.date <- "2024-04-22"# "2023-07-06" #"2020-10-14" #Sys.Date() #"2019-06-25" #
run.date.JM.only <- "2024-04-22" # "2023-07-06" #"2021-03-08"
...
The temporal nest count pattern at Wermon is different from that at Jamursba-Medi (Figure \@ref(fig:plot-JM-by-month) vs. Figure \@ref(fig:plot-W-by-month)). At Wermon, there are two peaks within each nesting season: higher values are found between May and September and between October and April.  
...
Annual patterns of Observed counts at Wermon before the 2006 season were not consistent with those since the 2006 season (Figure \@ref(fig:plot-W-by-month)). Because this analysis used observed patterns to impute missing values, we removed the data before 2006 from the data for the following analysis (Figure \@ref(fig:plot-W-by-month-2006)).  
...
if (save.fig)
  ggsave(filename = 'figures/annual_counts.png', 
         plot = p.annual.counts,
         dpi = 600, height = fig.height, 
         width = fig.width, 
         units = "in")
```
...
looic.table.out <- cbind(models.part.names, looic.table) %>%
  select(-c(model, looic, weights)) %>%
  rename(N = State, y = Observation)
...
p.imputed.2beaches <- ggplot(estimated.counts %>% filter(Season > 2010)) +
...
if (save.fig)
  ggsave(filename = 'figures/imputed_2beaches.png', 
         plot = p.imputed.2beaches,
         dpi = 600, height = fig.height, 
         width = fig.width, 
         units = "in")
...
The estimated mean growth rates (U) at two beaches were `r signif(U.stats[1,"mean"], digits = 2)` (SE = `r signif(U.stats[1, "sd"], digits = 2)`, p(U < 0.0) = `r signif(U.stats[1, "f"], digits = 2)`) for Jamursba Medi and for Wermon (Figure \@ref(fig:plot-complete-U)). 
...
As it was the case with the complete dataset, Model Set 2 (v2: annual nest abundance as a random variable) fit better than Model Set 1 (v1: annual nest abundance as a non-random variable) according to the Pareto k statistics. Among the 16 models within Model Set 2, the lowest LOOIC value was found for the model with Student's t distribution for abundance (N), Student's t distribution for observations (y), two growth parameters (2U), and with location specific variance (2Q). (Table \@ref(tab:looic-table-summer)). Gelman-Rubin Rhat statistics indicated successful convergence of all parameters (maximum Rhat = `r signif(rhat.max, digits=5)`. We use the joint posterior distributions from these two models for the inference.  
...
flextable(looic.table.out.winter) %>%
  set_caption("A comparison of models that were fit to the winter dataset and passed the LOOIC goodness-of-fit test using Pareto k statistics. In the N and y (observation) columns, norm and t indicate normal and Student's t distributions, respectively. Set indicates either Set 1 (v1) or Set 2 (v2) (see text for details), U indicates the number of U (slope) parameters and Q indicates the number of Q (variance) parameters. delta.looic indicates the difference in LOOIC values from the smallest LOOIC value, and Max.Pareto indicates the maximum Pareto k statistics value for each model.") %>%
   colformat_double(j = c("delta.looic", "Max.Pareto"),
                   digits = 3) %>%
  set_table_properties(width = 0.5, layout = "autofit")
...
df.JM.U <- data.frame(dataset = "Jeen Yessa",
                      U = JM.only$jm$sims.list$U,
                      Dataset.idx = "A")
...
flextable(looic.table.out.JM) %>%
  set_caption("A comparison of models that were fit to the Jamursba Medi dataset and passed the LOOIC goodness-of-fit test using Pareto k statistics. In the N and y (observation) columns, norm and t indicate normal and Student's t distributions, respectively. Set indicates either Set 1 (v1) or Set 2 (v2) (see text for details). delta.looic indicates the difference in LOOIC values from the smallest LOOIC value, and Max.Pareto indicates the maximum Pareto k statistics value for each model.") %>%
  colformat_double(j = c("delta.looic", "Max.Pareto"),
                   digits = 3) %>%
  set_table_properties(width = 0.5, layout = "autofit")
...
df.W.U <- data.frame(dataset = "Jeen Syuab",
                     U = W.only$jm$sims.list$U,
                     Dataset.idx = "B")

df.W.JM.U <- rbind(df.JM.U, df.W.U)

p.U.W.JM <- ggplot(df.W.JM.U) +
  geom_density(aes(x = U, y = after_stat(scaled))) +
  facet_wrap2(~Dataset.idx, ncol = 1,
              axes = "all",
...
if (save.fig)
  ggsave(filename = "figures/U_W_JM_posteriors.png",
         plot = p.U.W.JM,
         dpi = 600,
         height = fig.height,
         width = fig.width,
...
```{r plot-posterior-W-U, echo=FALSE,  fig.cap = "The posterior distribution of the growth rate (U) at Jamursba Medi (A) and Wermon (B) from the best model using data based on LOOIC. "}
knitr::include_graphics("figures/U_W_JM_posteriors.png")
```
