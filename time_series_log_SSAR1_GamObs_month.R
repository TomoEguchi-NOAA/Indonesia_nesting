...
jm <- jags.model(file = 'models/model_SSAR1_logN_GamObs_month.txt',
                 data = bugs.data,
...
toc <- Sys.time()
dif.time <- toc - tic
results.JM_SSAR1_logN_GamObs_month <- list(data.1 = data.1.JM.2005,
                                    summary.zm = summary.zm,
                                    Xs.stats = Xs.stats,
                                    Xs.year = Xs.year,
                                    ys.stats = ys.stats,
...
if (save.fig)
  ggsave(plot = p.1,
         filename = 'figures/predicted_SSAR1_logN_GamObs_month_JM.png',
         dpi = 600)

if (save.RData)
  save(results.JM_SSAR1_logN_GamObs_month,
       file = paste0('RData/SSAR1_logN_GamObs_month_', Sys.Date(), '.RData'))
