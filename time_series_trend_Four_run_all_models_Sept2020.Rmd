...
#  jags.data$Z <- Z
  ...
  for (k2 in 1:length(obs.models)){
    ...
    if (obs.model == "t" | obs.models == "t_locs"){
...
So, this is pretty much identical to the previous model (independent Us). LOOIC value for this one was only ```r looic[best.model.ID] - looic[oneU.model]``` greater than the other model (```r looic[best.model.ID]``` vs. ```r looic[oneU.model]```). So, it may make sense to use the simpler model.
