...
load('RData/SSAR1_month_JM_var_theta_2018-04-09_2000.RData')

Xs.stats <- results.JM_SSAR1_month$Xs.stats
ys.stats <- results.JM_SSAR1_month$ys.stats
zm <- results.JM_SSAR1_month$zm
...
NOt great but according to the Gelman diagnostic, the convergence was reached. 
...
```{r}
load("RData/SSAR1_month_Warmon_2018-04-23_To2013.RData")
...
```{r}
load("RData/SSAR1_month_Warmon_2018-04-23_2006To2013.RData")
...
Now, how about removing the first two years and include the two and a half years of missing data? 

```{r}
load("RData/SSAR1_month_Warmon_2018-04-23_2006To2017.RData")
...
Convergence was reached according to the Gelman diagnostic. The posterior of the SD of observation model seems okay.
...
```{r}
fit.gam.W <- gam(log(total) ~ s(year.0), 
                 data = dataTo2017.W)
...
Xs.year.W3 %>% mutate(log.total = log(total)) %>%
  mutate(log.total.std = log(total) - mean(log(total), na.rm = T)) -> Xs.year.W3
