...
if(!file.exists(paste0("Rdata/singleUQ_JM_", 
  ...
  jm.out$data <- thedata
  saveRDS(jm.out, file = paste0("Rdata/singleUQ_JM_", 
                                begin.year, "_", end.year,  "_", run.date, ".rds"))
  print(jm.out$out)

} else {
  jm.out <- readRDS(file = paste0("Rdata/singleUQ_JM_", 
                                begin.year, "_", end.year,  "_", run.date, ".rds"))
}

```

Majority (```r signif(jm.out$out$summary["U", "f"], 3) * 100``` %) of the posterior of U is less than 0, indicating a decline. 

```{r}
bayesplot::mcmc_dens(jm.out$out$samples, "U")
```
...
if(!file.exists(paste0("Rdata/singleUQ_W_2006_", 
  ...
  jm.out$data <- thedata
  saveRDS(jm.out, file = paste0("Rdata/singleUQ_W_2006_", 
                                end.year,  "_", run.date, ".rds"))
  print(jm.out$out)

} else {
  jm.out <- readRDS(file = paste0("Rdata/singleUQ_W_2006_", 
                                end.year,  "_", run.date, ".rds"))
}
...
The similar analysis can be done using independent-U and single-Q or independent-Q model output. Using the simpler of the two:
...
```{r}
bayesplot::mcmc_dens(jags.model$out$samples, c("U[1]", "U[2]"))

```

Majority (```r signif(jags.model$out$summary["U[1]", "f"], 3) * 100``` and ```r signif(jags.model$out$summary["U[2]", "f"], 3) * 100```%) of the posterior is less than 0, indicating a decline. 
...
Majority (```r signif(jags.model$out$summary["U", "f"], 3) * 100``` %) of the posterior is less than 0, indicating a decline. 
...
Look at the posterior of the slope parameter (U).
```{r}
bayesplot::mcmc_dens(jags.model_summer$out$samples, "U")
```

Majority of (```r signif(jags.model_summer$out$summary["U", "f"], 3) * 100```) % of the posterior is less than 0. 
...
Look at the posterior of the slope parameter (U).
```{r}
bayesplot::mcmc_dens(jags.model_winter$out$samples, "U")
```


Majority (```r signif(jags.model_winter$out$summary["U", "f"], 3) * 100``` %) of U is < 0. 
