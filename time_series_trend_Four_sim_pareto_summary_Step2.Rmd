...
run.date <- "2019-07-09"  # The date that step 1 was run. 
```

First load the results:
```{r}
all.results <- readRDS(file = paste0("RData/time_series_trend_Four_sim_pareto_no_missing_summary_", 
                                     run.date,  ".rds"))
...
p.1 <- ggplot(data = filter(pareto.df, loc == 1)) + 
  ...
  #                       range = c(1, 4))+
  scale_color_manual(values = c("<=0.7" = "darkblue",
                                ">0.7" = "red")) + 
...
```{r}
p.2 <- ggplot(data = filter(pareto.df, loc == 2)) + 
  geom_path(aes(x = datapoint, y = y), 
            alpha = 0.5) +
  geom_point(aes(x = datapoint, 
                 y = y,
