...
# the date jags was run (time_series_SSAR1_norm_norm_trend.R and time_series_SSAR1_norm_norm_trend_Four.R)
run.date <- "2019-04-10"
...
This document describes trend analysis of leatherback turtles at nesting beaches in Indonesia. Bayesian state-space time series analyses.  In this Take 2, I combined the data imputation process and the trend analysis using annual abundances. Two locations are modeled separately at the moment. 
...
p.1 <- bayesplot::mcmc_dens(jm.out$out$samples, "U")
p.1 <- p.1 + labs(y = "Density", x = "", title = "Jamursba-Medi (Summer, one dataset)") +
  xlim(c(-1.1, 1.1)) +
  ylim(c(0, posterior.lim.y)) + 
  geom_text(aes(x = 0.5, y = text.y), 
            label = U.stats, hjust =  "left") + 
...
if(!file.exists(paste0("Rdata/independentUs_singleQ_out_", 
  ...
  jm.out <- readRDS(file = "RData/independentUs_singleQ_out_2001_2018_2019-03-14.rds")
