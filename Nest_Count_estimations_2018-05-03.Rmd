...
#With new data
New dataset was provided in June 2018. 
```{r}
data.0.new <- read.csv("data/NestCounts_JM_W_DLmodified_Mei2018.csv")
data.0.new %>% select(Year_begin, Month_begin, Year_end, Month_end, J_M) %>%
  #na.omit() %>% 
  mutate(frac.yr.begin = Year_begin + (Month_begin - 0.5)/12,
         frac.yr.end = Year_end + (Month_end - 0.5)/12,
         n.month = 1 + 12 * (frac.yr.end - frac.yr.begin),
         f.year = as.factor(Year_begin)) %>%
  filter(n.month == 1) -> data.1.JM.new
data.0.new %>% select(Year_begin, Month_begin, Year_end, Month_end, W) %>% 
  #na.omit() %>% 
  mutate(frac.yr.begin = Year_begin + (Month_begin - 0.5)/12,
         frac.yr.end = Year_end + (Month_end - 0.5)/12,
         n.month = 1 + 12 * (frac.yr.end - frac.yr.begin),
         f.year = as.factor(Year_begin)) %>%
  filter(n.month == 1)-> data.1.W.new
...
p.JM.all <- ggplot() + 
  ...
  geom_line(data = data.1.JM,
            aes(x = Frac.Year, y = count)) +
  geom_point(data = data.1.JM.new,
             aes(x = frac.yr.begin, y = J_M),
             color = "seagreen2",
             alpha = 0.5) + 
  geom_line(data = data.1.JM.new,
            aes(x = frac.yr.begin, y = J_M),
            color = "seagreen2",
            alpha = 0.5) +
...
Using new data but remove 1981.
```{r}

p.JM.monthly.new <- ggplot(filter(data.1.JM.new, Year_begin > 1981)) + 
  geom_point(aes(x = Month_begin, y = J_M, color = f.year)) + 
  geom_line(aes(x = Month_begin, y = J_M, color = f.year)) +
  scale_x_continuous(breaks = seq(1, 12, 1)) +
  labs(x = 'Month', y = '# nests', 
       title = "Jamursba-Medi (new data)", color = "Year")  +
  theme(axis.text = element_text(size = 12),
        text = element_text(size = 12),
        legend.position = c(0.9, 0.6))
p.JM.monthly.new

if (save.fig) ggsave(filename = 'figures/JM_monthly_new.png', 
                     plot = p.JM.monthly,
                     dpi = 600, height = 6, 
                     width = 8, units = "in")
...
No data were collected in 1998. Since then at least some data were collected within each year. So, we look at the data since 1999. For the new dataset, it is since 2003.

```{r}
data.1.JM.1999 <- filter(data.1.JM, YEAR > 1998)
data.1.JM.new.2003 <- filter(data.1.JM.new, Year_begin > 2002)
p.JM.1999 <- ggplot() + 
  geom_point(data = data.1.JM.1999,
             aes(x = Frac.Year, y = count)) + 
  geom_line(data = data.1.JM.1999,
            aes(x = Frac.Year, y = count)) +
  geom_point(data = data.1.JM.new.2003,
             aes(x = frac.yr.begin, y = J_M),
             color = "seagreen2",
             alpha = 0.5) + 
  geom_line(data = data.1.JM.new.2003,
            aes(x = frac.yr.begin, y = J_M),
            color = "seagreen2",
            alpha = 0.5) +
...
Using the cyclic nature of nesting, we should be able to fit a model to fill in the missing data points. We need to make some assumptions about how data were collected, unless we have a measure of effort every year.  I ran time-series models to compare how they do in terms of filling in the missing data points. Here I try to fill in the missing data points using an AR(1) state-space model and data since 1999. This model has two variance and two "slope" parameters. Two variances correspond to high nesting (May, June, July, August, and September; sigma.pro1) and low nesting (sigma.pro2) seasons, whereas two slopes are for increasing (Jan to Jul) and decreasing (Aug to Dec) phases of each year.  Because it takes a bit of time to run these, output has been saved as an RData file. 
